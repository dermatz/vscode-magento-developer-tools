...
export function copyCommand(context: vscode.ExtensionContext) {
    let disposable = vscode.commands.registerCommand('magento-developer-tools.copyToThemeLocation', async () => {
        let magentoThemeLocations: string[] = vscode.workspace.getConfiguration('magento-developer-tools').get('magentoThemeLocations') || [];
        if (magentoThemeLocations) {
            ...
            if (location) {
                ...
                if (editor) {
                    ...
                    let destinationUri = vscode.Uri.file(`${workspaceFolder}` + location + '/' + document.fileName.split('/').pop());
                    vscode.window.showInformationMessage(`Copying ${document.fileName} to ${location}`);
                    ...
                    Promise.resolve(
                        ...
                        .then(() => vscode.window.showInformationMessage(`Copied to ${location}`))
                        .catch((err) => vscode.window.showErrorMessage(`Failed to copy: ${err}`));
                }
            }
        }
    });
